...
.table-slipper th {
    text-align: center; /* Center align the column names */
}

/* Adjust the width of the Name column */
.table-slipper th:first-child,
.table-slipper td:first-child {
    width: 200px; /* Set a specific width for the Name column */
    text-align: center; /* Center align the content in the Name column */
}
/* Adjust the width of the Location column */
.table-slipper th:nth-child(3),
.table-slipper td:nth-child(3) {
    width: 200px; /* Set a specific width for the Location column */
    text-align: center; /* Center align the content in the Location column */
}

/* Center align the content within the Location column */
.table-slipper td:nth-child(3) {
    vertical-align: middle; /* Center align vertically */
}

/* Adjust the width of the Adult Fee and Child Fee columns */
.table-slipper th:nth-child(4),
.table-slipper td:nth-child(4),
.table-slipper th:nth-child(5),
.table-slipper td:nth-child(5),
.table-slipper th:nth-child(6),
.table-slipper td:nth-child(6) {
    width: 150px; /* Set a specific width for both Adult Fee, Child Fee, and Information columns */
    text-align: center; /* Center align the content in these columns */
}

/* Match styles for .tab-pane.fade, .table-responsive, and .dropdown-menu */
.tab-pane.fade {
...
<body>
        ...
        <a href="/Super-admin-main/adventours/usersrestaurant.php" class="btn active" onclick="toggleButtonUnderline(this, 'restaurants'); return true;">Restaurants</a> 
    </div>
...
<body class="p-0">
    ...
    ?>
            <div class="tab-content" id="myTabContent">
                ...
                <table class="table table-slipper text-center center-table big-table" style="width: 120%; margin-left:20px;">
                        <thead>
                            <tr>
                                <th>Name</th>
                                <th>Email</th>
...
function closeModal() {
    ...
    function GetAllDataOnce() {
        db.collection("Restaurants").get().then((querySnapshot) => {
            ...
            AddAllItemsToTheTable(Restaurants);
        });
    }

   // Function to add items to the table
function AddItemToTable(name, email, location, adult_fee, child_fee, desc, gcash_account_name, gcash_account_number, maya_account_name, maya_account_number) {
...
function AddAllItemsToTheTable(RestaurantsDocsList) {
    tbody.innerHTML = ""; // Clear table body
    RestaurantsDocsList.forEach(element => {
        AddItemToTable(element.name, element.email, element.location, element.adult_fee, element_child, element.desc, element.gcash_account_name, element.gcash_account_number, element.maya_account_name, element.maya_account_number);
    });
}
    </script>
    ...
    td2.textContent = email;
    td3.textContent = location;
    td4.textContent = adult_fee;
    td5.textContent = child_fee;
...
function AddAllItemsToTheTable(RestaurantsDocsList) {
    tbody.innerHTML = ""; // Clear table body
    RestaurantsDocsList.forEach(element => {
        AddItemToTable(element.name, element.email, element.location, element.adult_fee, element.child_fee, element.desc, element.gcash_account_name, element.gcash_account_number, element.maya_account_name, element.maya_account_number);
    });
}
