...
<script>
    ...
    function AddItemToTable(reservationId, CustomerName, RestaurantName, Guests, CheckIn, CheckOut, receipt, status, action) {
...
function notifyUser(status) {
    ...
    function AddAllItemsToTheTable(RestaurantReservationDocsList) {
        ...
        RestaurantReservationDocsList.forEach(element => {
            AddItemToTable(element.reservationId, element.CustomerName, element.RestaurantName, element.Guests, element.CheckIn, element.CheckOut, element.receipt, element.status, element.action);
        });
    }
